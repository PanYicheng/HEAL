...
class GrangerCausalRunner:
    ...
    def pairwise_process(self, param_dict, specific_params, result_dict):
        ...
        """
        if param_dict['use_multiprocess']:
            if param_dict['use_shared_mem']:
                # Load from shared_memory
                shm_worker = shared_memory.SharedMemory(param_dict['data'])
                shm_worker_arr = np.ndarray(param_dict['size'], dtype=np.float64, buffer=shm_worker.buf)
                data = shm_worker_arr.copy()
                shm_worker.close()
            else:
        ...
        try:
            ...
            print(e)
            result_dict["{}->{}".format(x_i, y_i)] = (None, None, None)
...
class TemporalAnalyze:
    ...
    def create_shared_data(self, data, time_str=None):
        if self.use_shared_mem:
            shm_master = shared_memory.SharedMemory(create=True, size=data.nbytes)
            shm_master_arr = np.ndarray(data.shape, dtype=np.float64, buffer=shm_master.buf)
            shm_master_arr[:] = data.astype(np.float64)[:]
            shared_data_param = {
                "data": shm_master.name,
                'size': data.shape
            }
            self.shm_master = shm_master
            return shared_data_param
        else:
            if time_str is None:
            ...
            self.shared_data_param = shared_data_param
            safe_dump_obj(data, shared_data_param["data"])
            return shared_data_param

    def remove_shared_data(self):
        if self.use_shared_mem:
            self.shm_master.close()
            self.shm_master.unlink()
        else:
            os.remove(self.shared_data_param['data'])
    ...
    def granger_analyze(self, data, use_multiprocess=True, mp_mode=2, **kws):
        ...
        if (
            ...
            param_dict = {
                'use_shared_mem': self.use_shared_mem,
                'use_multiprocess': use_multiprocess
            }
            
            if use_multiprocess and max_workers > 1:
                shared_data_param = self.create_shared_data(data, time_str=time_str)
                param_dict.update(shared_data_param)
                if mp_mode == 1:
                    ...
                    # But each process only does its own jobs. This way may use less time in creating and destroying processes.
                    param_dict["N"] = N
                ...
                # After all process, remove the shared data object (SharedMemory or Disk file)
            ...
            for x_i in range(N):
                for y_i in range(N):
                    ...
                    # If returned None results, means Granger test failed, we skip it.
                    if array_results_YX is None or array_results_XY is None:
                        continue
        ...
        for x_i in range(N):
            for y_i in range(N):
                if y_i == x_i:
                    continue
                key = f"{x_i}->{y_i}"
                # If local_results doesnot has a valid entry, means the Granger test
                # failed, we do not generate the dcc either.
                if key not in self.local_results:
                    continue
