...
@author: Amal
@Modified by: Pan Yicheng on 2020/12/9
@Modified by: Pan Yicheng on 2021/04/14. Add a new function `loop_granger_pair` that
  wraps around `loop_granger`. The new function requires two time series. 
  Also simplify irrelavant parameters.
...
def loop_granger_pair(
    array_feature, array_target, dir_output,
    significant_thres, test_mode, trip, lag, step, simu_real,
    max_segment_len, min_segment_len, verbose=True, return_result=False):
    """针对一对时序数据进行Granger causal interval计算。
    本函数通过包装loop_granger实现。

    Args:
        array_feature : 1d time series for feature varialbe, a.k.a X. Shape [T, 1].
        array_target : 1d time series for target varialbe, a.k.a Y. Shape [T, 1].
        significant_thres : significant value
        test_mode : same as loop_granger
        trip : same as loop_granger
        lag : same as loop_granger
        step : same as loop_granger
        simu_real : same as loop_granger
        max_segment_len : same as loop_granger
        min_segment_len : same as loop_granger
        verbose (bool, optional): whether print detailed info. Defaults to True.
        return_result (bool, optional): whether return result p value matrix. 
            Defaults to False.
    Returns:
        根据参数return_result返回不同值：
        当return_result=True时：total_time, time_OLS, time_granger, time_adf, array_results_YX, array_results_XY
        当return_result=False时：total_time, time_OLS, time_granger, time_adf
    """
    array_data = np.hstack([array_feature, array_target])
    array_data_head = ['X', 'Y']
    return loop_granger(array_data, array_data_head, dir_output, 'X', 'Y',
                 significant_thres, test_mode, trip, lag, step, simu_real,
                 max_segment_len, min_segment_len, verbose=verbose, return_result=return_result)
...
def loop_granger(array_data, array_data_head, path_to_output, feature, target,
    ...
    Returns:
        根据参数return_result返回不同值：
        当return_result=True时：time_dict, array_results_YX, array_results_XY
        当return_result=False时：time_dict
            time_dict is a dict with keys ['total_time', 
                'time_OLS', 'time_granger','time_adf', 'time_window',
                'cnt_prune_YX', 'cnt_prune_XY']
    """
    addconst = True
    # in seconds
    time_diff_thres = 60
    ...
    for i in range(n_step):
        ...
        for j in range(i + 1, n_step + 1):
            ...
            if len(array_YX[start:end, :]) < min_segment_len \
                ...
                # print(' Skipped', len(array_YX[start:end, :]))
                continue
            # print(' Checked')
            time3 = timeit.default_timer()
            # check YX then check XY
            if test_mode == 'fast_version_3':
                # granger test (fast)
                ...
                (p_value_YX, res2down_YX, res2djoint_YX,
                 ...
                 res2djoint_df_resid_YX, reset_cnt_YX, cnt_prune_YX, t_OLS) \
                ...
                if p_value_YX < significant_thres and p_value_YX != -1:
                    ...
                    (p_value_XY, res2down_XY, res2djoint_XY,
                     ...
                     res2djoint_df_resid_XY, reset_cnt_XY, cnt_prune_XY, t_OLS) \
                        = grangercausalitytests_check_F_upper_lower(
    ...
    if verbose:
        print('Prune for Y <-- X:\n', ' '*5, cnt_prune_YX)
        print('Prune for X <-- Y:\n', ' '*5, cnt_prune_XY)
    time_stat = {'total_time': total_time, 
                'time_OLS': time_OLS, 
    ...
    if return_result:
        return time_stat, array_results_YX, array_results_XY
    return time_stat
...
def grangercausalitytests_check_F_upper_lower(x,
    ...
    if cnt == -1:
        ...
        return (p_value, res2down, res2djoint, res2down_ssr_upper,
                ...
                res2djoint.df_resid, 0, cnt_prune, t_OLS)
    else:
        # fit the new data
        # prune promising not
        ...
        if p_value_lower >= significant_thres:  # promising not
            # print(f"p_lower={p_value_lower} f_upper={F_upper} df_resid={res2djoint_df_resid}")
            ...
            return (p_value, pre_res2down, pre_res2djoint,
                    ...
                    res2djoint_ssr_upper, res2djoint_ssr_lower,
                    res2djoint_df_resid, cnt + 1, cnt_prune, 0)
            ...
            if p_value_upper < significant_thres:
                ...
                return (p_value, res2down, res2djoint,
                        res2down_ssr_upper, res2down_ssr_lower,
                        res2djoint_ssr_upper, res2djoint_ssr_lower,
                        res2djoint.df_resid, cnt+1, cnt_prune, t_OLS)
