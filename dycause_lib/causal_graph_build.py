...
def get_ordered_intervals(matrics, significant_thres, list_segment_split, level=None):
    ...
    for i in range(nrows):
        for j in range(i + 1, ncols):
            ...
            if (abs(array_results_YX[i, j]) < significant_thres) and ( 
                ...
                or array_results_XY[i, j] < 0 # p < 0 means that the actual value is not computed. So we also assume the causality does not exist.
            ):
